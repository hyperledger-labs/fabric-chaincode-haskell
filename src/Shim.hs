...
import           Interfaces                       ( ChaincodeStubInterface(..), StateQueryIteratorInterface(..) )
...
chatWithPeer
    :: IO (Either GRPCIOError (Maybe ChaincodeMessage)) -> StreamSend ChaincodeMessage -> ChaincodeStub -> IO b
...
handler :: ChaincodeMessage -> StreamRecv ChaincodeMessage -> StreamSend ChaincodeMessage -> ChaincodeStub -> IO ()
handler message recv send chaincodeStub = case message of
    ...
    ChaincodeMessage{chaincodeMessageType = Enumerated (Right ChaincodeMessage_TypeINIT)} ->
        trace "INIT message received from the peer" $ handleInit message recv send (initFn chaincodeStub)
    ChaincodeMessage{chaincodeMessageType = Enumerated (Right ChaincodeMessage_TypeTRANSACTION)} ->
        trace "TRANSACTION message received from the peer" $ handleInvoke message recv send (invokeFn chaincodeStub)
    ...
    in
        case eStub of
            ...
            Right stub -> do
                ...
                e <- send (buildChaincodeMessage COMPLETED response (getTxId stub) (getChannelId stub))
                    :: IO (Either GRPCIOError ())
                case e of
                    Left err -> error ("Error while streaming: " ++ show err)
                    Right _ -> pure ()

handleInvoke :: ChaincodeMessage
             -> StreamRecv ChaincodeMessage
             -> StreamSend ChaincodeMessage
             -> (DefaultChaincodeStub -> IO Pb.Response)
             -> IO ()
...
newChaincodeStub :: ChaincodeMessage
                 -> StreamRecv ChaincodeMessage
                 -> StreamSend ChaincodeMessage
        ...
        in
            case maybeSignedProposal of
                ...
                Just signedProposal -> do
                    ...
                    Right $ DefaultChaincodeStub { args           = chaincodeInputArgs input
                                                 , txId           = toStrict $ chaincodeMessageTxid mes
                                                 , channelId      = toStrict $ chaincodeMessageChannelId mes
                                                 ...
                                                 , transient      = Just $
                                                       chaincodeProposalPayloadTransientMap chaincodeProposalPayload
